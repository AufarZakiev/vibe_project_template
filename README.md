# 2030AI Project Template

Шаблон проекта для разработки с Claude Code.

## Важно

Перед началом работы самостоятельно или с помощью агента заполните блок "Описание проекта и целей пользователя" в `AGENTS.md` — это основной контекст для агента.

## Структура проекта

```
├── AGENTS.md                 # Универсальные правила для всех агентов
├── CLAUDE.md                 # Настройки для Claude Code (ссылается на AGENTS.md)
├── .gitignore                # Настроен для macOS/Windows/Linux, IDE, Python, Node.js
├── .claude/                  # Конфигурация Claude Code
│   ├── settings.json         # Права и ограничения (запрет git-команд)
│   └── commands/             # Slash-команды
│       ├── security-review.md  # /project:security-review — аудит безопасности
│       └── lint.md             # /project:lint — запуск линтеров
└── agent_docs/               # Проектная документация
    ├── index.md              # Навигация по документам
    ├── architecture.md       # Архитектура и компоненты
    ├── adr.md                # Архитектурные решения
    ├── development-history.md # История разработки
    ├── guides/               # Гайды (DoD, окружение, логирование)
    └── templates/            # Шаблоны документов
```

## Slash-команды

| Команда | Описание |
|---------|----------|
| `/project:security-review` | Аудит безопасности кода — находит уязвимости, объясняет риски, предлагает исправления |
| `/project:lint` | Запуск линтера — определяет тип файла, проверяет наличие линтера, запускает или предлагает установку |

## Быстрый старт

1. Клонируйте или используйте как template репозиторий
2. Заполните блок описания проекта в `AGENTS.md`
3. Ознакомьтесь с навигацией в `agent_docs/index.md`
4. Начните разработку

## Документация

- `AGENTS.md` — правила работы и чек-листы
- `CLAUDE.md` — инструкции для Claude Code (стиль кода, правила окружения)
- `agent_docs/index.md` — карта всех документов
